...
zoomRefGlobal=function(d){window.appModel.zoomRef(d)},starFigureGlobal=function(d){window.appModel.starFigure(d)};$(window).resize(function(){500>window.innerWidth?($("#map").height(window.innerHeight-80),$(".tab-content").height(window.innerHeight-100)):$("#map").height(window.innerHeight-115)});
...
url:c,dataType:"xml",success:function(b){$imgxml=$(b).find("IMAGE_PROPERTIES");var b=$imgxml.attr("HEIGHT"),a=$imgxml.attr("WIDTH");imageHeight=b;imageWidth=a;var c=Math.ceil(Math.log(Math.max(a,b)/256)/Math.log(2));500>window.innerWidth?($("#map").height(window.innerHeight-80),$(".tab-content").height(window.innerHeight-100)):($("#map").height(window.innerHeight-115),$(".tab-content").height(window.innerHeight-200));map=new L.Map("map",{crs:L.CRS.Direct,center:new L.LatLng(0,0),zoom:c,minZoom:1,
scrollWheelZoom:!0,worldCopyJump:!1,attributionControl:!1});$("#activityView").hide().ajaxStart(function(){$(this).show()}).ajaxStop(function(){$(this).hide()});var g=new L.Control.FullScreen;map.addControl(g);g=L.Control.extend({options:{position:"topright"},_createButton:function(a,b,c,f,i){b=L.DomUtil.create("a",b,c);b.href="#";b.title=a;L.DomEvent.addListener(b,"click",L.DomEvent.stopPropagation).addListener(b,"click",L.DomEvent.preventDefault).addListener(b,"click",f,i);return b},onAdd:function(){var a=
L.DomUtil.create("div","leaflet-control-draw");this._createButton("reset zoom","leaflet-control-draw-zoom",a,resetMap,window);return a}});map.addControl(new g);map.setView(map.layerPointToLatLng(new L.Point($("#map").width(),$("#map").height())),c,!0);L.TileLayer.FigureZero=L.TileLayer.Common.extend({url:e+"TileGroup0/{z}-{x}-{y}.jpg",options:{noWrap:!0,maxZoom:c}});c=new L.TileLayer.FigureZero;map.addLayer(c);tlpoint=map.layerPointToLatLng(new L.Point(0,b));tlpoint.lat*=0.1;brpoint=map.layerPointToLatLng(new L.Point(a,
0));brpoint.lng*=0.1;regionArray=$("#initialRegion").val().split(",");console.log(regionArray);3>regionArray.length?(console.log("loading default"),map.fitBounds(new L.LatLngBounds(tlpoint,brpoint))):(console.log("loading initial rect"),b=new L.LatLng(regionArray[0],regionArray[1]),a=new L.LatLng(regionArray[2],regionArray[3]),console.log(tlpoint),console.log(b),map.fitBounds(new L.LatLngBounds(b,a)));var i=L.icon({iconUrl:"/static/css/images/pubmed-marker.png",iconSize:[32,37],iconAnchor:[16,37],
popupAnchor:[0,-28]}),d=L.icon({iconUrl:"/static/css/images/doi-marker.png",iconSize:[32,37],iconAnchor:[16,37],popupAnchor:[0,-28]}),j=L.icon({iconUrl:"/static/css/images/vim-marker.png",iconSize:[32,37],iconAnchor:[16,37],popupAnchor:[0,-28]}),l=L.icon({iconUrl:"/static/css/images/yt-marker.png",iconSize:[32,37],iconAnchor:[16,37],popupAnchor:[0,-28]}),r=L.icon({iconUrl:"/static/css/images/other-marker.png",iconSize:[32,37],iconAnchor:[16,37],popupAnchor:[0,-28]}),m=L.icon({iconUrl:"/static/css/images/highlight-marker.png",
iconSize:[32,37],iconAnchor:[16,37],popupAnchor:[0,-28]});geojsonViewLayer=L.geoJson(null,{onEachFeature:onEachRectFeature,style:{color:"#0099ff",weight:2,opacity:0.8,fillColor:"#000000",fillOpacity:0.75}}).addTo(map);pubmedLayer=L.geoJson(null,{pointToLayer:function(a,b){return L.marker(b,{icon:i})},onEachFeature:onEachPointFeature}).addTo(map);dxLayer=L.geoJson(null,{pointToLayer:function(a,b){return L.marker(b,{icon:d})},onEachFeature:onEachPointFeature}).addTo(map);ytLayer=L.geoJson(null,{pointToLayer:function(a,
b){return L.marker(b,{icon:l})},onEachFeature:onEachPointFeature}).addTo(map);vimLayer=L.geoJson(null,{pointToLayer:function(a,b){return L.marker(b,{icon:j})},onEachFeature:onEachPointFeature}).addTo(map);urlLayer=L.geoJson(null,{pointToLayer:function(a,b){return L.marker(b,{icon:r})},onEachFeature:onEachPointFeature}).addTo(map);refhighlightLayer=L.geoJson(null,{pointToLayer:function(a,b){return L.marker(b,{icon:m})},onEachFeature:onEachPointFeature}).addTo(map);window.appModel.addAllRefs();window.appModel.addAllView();
window.appModel.addFigureInfo()}})},layerToGeometry=function(d){var c,e=[];if(L.Marker&&d instanceof L.Marker)return c="Point",d=LatLngToCoords(d._latlng),{type:c,coordinates:d};if(L.Polygon&&d instanceof L.Polygon)return c="Polygon",e=LatLngsToCoords(d._latlngs,1),{type:c,coordinates:[e]};if(L.Polyline&&d instanceof L.Polyline)return c="LineString",e=LatLngsToCoords(d._latlngs),{type:c,coordinates:e}},LatLngToCoords=function(d,c){var e=parseFloat(c?d.lng:d.lat);return[parseFloat(c?d.lat:d.lng),e]},
LatLngsToCoords=function(d,c,e){var b,a=[],f,g;f=0;for(g=d.length;f<g;f++)b=c?LatLngToCoords(d[f],c-1,e):LatLngToCoords(d[f],e),a.push(b);return a};function onEachRectFeature(d){d.properties&&d.properties.title&&(new L.LatLng(d.geometry.coordinates[0][3][1],d.geometry.coordinates[0][3][0]),new L.LatLng(d.geometry.coordinates[0][1][1],d.geometry.coordinates[0][1][0]))}
...
{maxWidth:250}):-1!=d.properties.title.indexOf("iframe")?c.bindPopup('<iframe id="ytplayer" type="text/html" width="500" height="500" src="'+d.properties.id+'" frameborder="0"/>',{maxWidth:500}):c.bindPopup('<h3><span class="heavier">'+d.properties.title+"</span></h3><br>"+d.properties.authors+"<br>"+d.properties.description+'</p><p><a target="_blank" href="'+d.properties.id+' ">Original URL</a></p>',{maxWidth:500});var e=new L.LatLng(d.geometry.coordinates[1],d.geometry.coordinates[0]);map.panTo(e);
c.openPopup()})}
$(function(){Parse.$=jQuery;Parse.initialize("nY5yXtyaq5zJ1TnCfGOBDa6vu9IX9WwhDJTxx6eu","2HnPlp2kBNpS66dsPdVHRlec1b8BVzKnbZc5XknC");var d=Parse.Object.extend("UploadObject",{initialize:function(){console.log("initialized FZ object")},toggle:function(){console.log("called Toggle")}});Parse.Collection.extend({model:d});var c=Parse.Object.extend("AppState",{defaults:{filter:"all"}}),e=Parse.View.extend({tagName:"div",template:_.template($("#reference-template").html()),events:{},initialize:function(){_.bindAll(this,"render")},
...
if(b.has("starArray")){var g=$.inArray(a,b.get("starArray"));console.log(g);0<=g?($.get(f,function(a){console.log(a)}),b.get("starArray").splice(g,1),b.save(null,{success:function(){var a='<a class="btn btn-small" onclick="starFigureGlobal(\''+activeFigure.id+'\')"><span class="icon-star"></span></a>';$("#starSpan").html(a)}})):($.get(c,function(a){console.log(a)}),b.get("starArray").push(a),b.save(null,{success:function(){var a='<a class="btn btn-warning btn-small" onclick="starFigureGlobal(\''+
...
$("#editRegionSpan").html("<div class='clearfix'></div>");$("#editReferenceSpan").html("<div class='clearfix'></div>")},error:function(){}}))},saveCurrentText:function(){var a=this,b=a.$("#figureTitleEDIT").val(),c=a.$("#figureDescriptionEDIT").val(),f=a.$("#figureExternalEDIT").val(),g=a.$("#textarea").textext()[0].tags()._formData,d=a.$("#figureAuthorList").val();activeFigure.set("tag",g);activeFigure.set("figureTitle",b);activeFigure.set("figureDescription",c);activeFigure.set("figureExternal",
...
"Vimeo ID"==a.properties.reftype?vimLayer.addData(a):"Other URL"==a.properties.reftype&&urlLayer.addData(a)}))},logOut:function(){Parse.User.logOut();window.appModel=new a;this.undelegateEvents();delete this},reloadImageView:function(){resetMap()},render:function(){this.delegateEvents()}}),f=Parse.View.extend({el:$("#todoapp"),initialize:function(){this.render()},render:function(){window.appModel=new a;$("#myTab a").click(function(a){a.preventDefault();$(this).tab("show")})}}),g=Parse.Router.extend({routes:{"region/:id":"showRegion",
"*actions":"defaultRoute"},initialize:function(){},showRegion:function(a){console.log("viewing region "+a);$("#initialRegion").val(a)},defaultRoute:function(a){console.log(a)}});new c;new g;Parse.history.start();new f});(function(){function d(a,b,c){if(a===b)return 0!==a||1/a==1/b;if(null==a||null==b)return a===b;a._chain&&(a=a._wrapped);b._chain&&(b=b._wrapped);if(a.isEqual&&h.isFunction(a.isEqual))return a.isEqual(b);if(b.isEqual&&h.isFunction(b.isEqual))return b.isEqual(a);var f=k.call(a);if(f!=k.call(b))return!1;switch(f){case "[object String]":return a==""+b;case "[object Number]":return a!=+a?b!=+b:0==a?1/a==1/b:a==+b;case "[object Date]":case "[object Boolean]":return+a==+b;case "[object RegExp]":return a.source==
b.source&&a.global==b.global&&a.multiline==b.multiline&&a.ignoreCase==b.ignoreCase}if("object"!=typeof a||"object"!=typeof b)return!1;for(var g=c.length;g--;)if(c[g]==a)return!0;c.push(a);var g=0,i=!0;if("[object Array]"==f){if(g=a.length,i=g==b.length)for(;g--&&(i=g in a==g in b&&d(a[g],b[g],c)););}else{if("constructor"in a!="constructor"in b||a.constructor!=b.constructor)return!1;for(var e in a)if(h.has(a,e)&&(g++,!(i=h.has(b,e)&&d(a[e],b[e],c))))break;if(i){for(e in b)if(h.has(b,e)&&!g--)break;
i=!g}}c.pop();return i}var c=this,e=c._,b={},a=Array.prototype,f=Object.prototype,g=a.slice,i=a.unshift,k=f.toString,j=f.hasOwnProperty,l=a.forEach,r=a.map,m=a.reduce,s=a.reduceRight,B=a.filter,C=a.every,D=a.some,v=a.indexOf,E=a.lastIndexOf,f=Array.isArray,H=Object.keys,w=Function.prototype.bind,h=function(a){return new t(a)};"undefined"!==typeof exports?("undefined"!==typeof module&&module.exports&&(exports=module.exports=h),exports._=h):c._=h;h.VERSION="1.3.3";var n=h.each=h.forEach=function(a,
c,f){if(null!=a)if(l&&a.forEach===l)a.forEach(c,f);else if(a.length===+a.length)for(var g=0,d=a.length;g<d&&!(g in a&&c.call(f,a[g],g,a)===b);g++);else for(g in a)if(h.has(a,g)&&c.call(f,a[g],g,a)===b)break};h.map=h.collect=function(a,b,c){var f=[];if(null==a)return f;if(r&&a.map===r)return a.map(b,c);n(a,function(a,q,g){f[f.length]=b.call(c,a,q,g)});a.length===+a.length&&(f.length=a.length);return f};h.reduce=h.foldl=h.inject=function(a,b,c,f){var g=2<arguments.length;null==a&&(a=[]);if(m&&a.reduce===
m)return f&&(b=h.bind(b,f)),g?a.reduce(b,c):a.reduce(b);n(a,function(a,q,d){g?c=b.call(f,c,a,q,d):(c=a,g=!0)});if(!g)throw new TypeError("Reduce of empty array with no initial value");return c};h.reduceRight=h.foldr=function(a,b,c,f){var g=2<arguments.length;null==a&&(a=[]);if(s&&a.reduceRight===s)return f&&(b=h.bind(b,f)),g?a.reduceRight(b,c):a.reduceRight(b);var d=h.toArray(a).reverse();f&&!g&&(b=h.bind(b,f));return g?h.reduce(d,b,c,f):h.reduce(d,b)};h.find=h.detect=function(a,b,c){var f;F(a,function(a,
q,g){if(b.call(c,a,q,g))return f=a,!0});return f};h.filter=h.select=function(a,b,c){var f=[];if(null==a)return f;if(B&&a.filter===B)return a.filter(b,c);n(a,function(a,q,g){b.call(c,a,q,g)&&(f[f.length]=a)});return f};h.reject=function(a,b,c){var f=[];if(null==a)return f;n(a,function(a,q,g){b.call(c,a,q,g)||(f[f.length]=a)});return f};h.every=h.all=function(a,c,f){var g=!0;if(null==a)return g;if(C&&a.every===C)return a.every(c,f);n(a,function(a,q,d){if(!(g=g&&c.call(f,a,q,d)))return b});return!!g};
var F=h.some=h.any=function(a,c,f){c||(c=h.identity);var g=!1;if(null==a)return g;if(D&&a.some===D)return a.some(c,f);n(a,function(a,q,d){if(g||(g=c.call(f,a,q,d)))return b});return!!g};h.include=h.contains=function(a,b){return null==a?!1:v&&a.indexOf===v?-1!=a.indexOf(b):F(a,function(a){return a===b})};h.invoke=function(a,b){var c=g.call(arguments,2);return h.map(a,function(a){return(h.isFunction(b)?b||a:a[b]).apply(a,c)})};h.pluck=function(a,b){return h.map(a,function(a){return a[b]})};h.max=function(a,
b,c){if(!b&&h.isArray(a)&&a[0]===+a[0])return Math.max.apply(Math,a);if(!b&&h.isEmpty(a))return-Infinity;var f={computed:-Infinity};n(a,function(a,q,g){q=b?b.call(c,a,q,g):a;q>=f.computed&&(f={value:a,computed:q})});return f.value};h.min=function(a,b,c){if(!b&&h.isArray(a)&&a[0]===+a[0])return Math.min.apply(Math,a);if(!b&&h.isEmpty(a))return Infinity;var f={computed:Infinity};n(a,function(a,q,g){q=b?b.call(c,a,q,g):a;q<f.computed&&(f={value:a,computed:q})});return f.value};h.shuffle=function(a){var b=
[],c;n(a,function(a,f){c=Math.floor(Math.random()*(f+1));b[f]=b[c];b[c]=a});return b};h.sortBy=function(a,b,c){var f=h.isFunction(b)?b:function(a){return a[b]};return h.pluck(h.map(a,function(a,b,q){return{value:a,criteria:f.call(c,a,b,q)}}).sort(function(a,b){var c=a.criteria,f=b.criteria;return void 0===c?1:void 0===f?-1:c<f?-1:c>f?1:0}),"value")};h.groupBy=function(a,b){var c={},f=h.isFunction(b)?b:function(a){return a[b]};n(a,function(a,b){var q=f(a,b);(c[q]||(c[q]=[])).push(a)});return c};h.sortedIndex=
function(a,b,c){c||(c=h.identity);for(var f=0,g=a.length;f<g;){var d=f+g>>1;c(a[d])<c(b)?f=d+1:g=d}return f};h.toArray=function(a){return!a?[]:h.isArray(a)||h.isArguments(a)?g.call(a):a.toArray&&h.isFunction(a.toArray)?a.toArray():h.values(a)};h.size=function(a){return h.isArray(a)?a.length:h.keys(a).length};h.first=h.head=h.take=function(a,b,c){return null!=b&&!c?g.call(a,0,b):a[0]};h.initial=function(a,b,c){return g.call(a,0,a.length-(null==b||c?1:b))};h.last=function(a,b,c){return null!=b&&!c?
g.call(a,Math.max(a.length-b,0)):a[a.length-1]};h.rest=h.tail=function(a,b,c){return g.call(a,null==b||c?1:b)};h.compact=function(a){return h.filter(a,function(a){return!!a})};h.flatten=function(a,b){return h.reduce(a,function(a,c){if(h.isArray(c))return a.concat(b?c:h.flatten(c));a[a.length]=c;return a},[])};h.without=function(a){return h.difference(a,g.call(arguments,1))};h.uniq=h.unique=function(a,b,c){var c=c?h.map(a,c):a,f=[];3>a.length&&(b=!0);h.reduce(c,function(c,g,d){if(b?h.last(c)!==g||
!c.length:!h.include(c,g))c.push(g),f.push(a[d]);return c},[]);return f};h.union=function(){return h.uniq(h.flatten(arguments,!0))};h.intersection=h.intersect=function(a){var b=g.call(arguments,1);return h.filter(h.uniq(a),function(a){return h.every(b,function(b){return 0<=h.indexOf(b,a)})})};h.difference=function(a){var b=h.flatten(g.call(arguments,1),!0);return h.filter(a,function(a){return!h.include(b,a)})};h.zip=function(){for(var a=g.call(arguments),b=h.max(h.pluck(a,"length")),c=Array(b),f=
0;f<b;f++)c[f]=h.pluck(a,""+f);return c};h.indexOf=function(a,b,c){if(null==a)return-1;var f;if(c)return c=h.sortedIndex(a,b),a[c]===b?c:-1;if(v&&a.indexOf===v)return a.indexOf(b);c=0;for(f=a.length;c<f;c++)if(c in a&&a[c]===b)return c;return-1};h.lastIndexOf=function(a,b){if(null==a)return-1;if(E&&a.lastIndexOf===E)return a.lastIndexOf(b);for(var c=a.length;c--;)if(c in a&&a[c]===b)return c;return-1};h.range=function(a,b,c){1>=arguments.length&&(b=a||0,a=0);for(var c=arguments[2]||1,f=Math.max(Math.ceil((b-
a)/c),0),g=0,d=Array(f);g<f;)d[g++]=a,a+=c;return d};var G=function(){};h.bind=function(a,b){var c,f;if(a.bind===w&&w)return w.apply(a,g.call(arguments,1));if(!h.isFunction(a))throw new TypeError;f=g.call(arguments,2);return c=function(){if(!(this instanceof c))return a.apply(b,f.concat(g.call(arguments)));G.prototype=a.prototype;var d=new G,i=a.apply(d,f.concat(g.call(arguments)));return Object(i)===i?i:d}};h.bindAll=function(a){var b=g.call(arguments,1);0==b.length&&(b=h.functions(a));n(b,function(b){a[b]=
h.bind(a[b],a)});return a};h.memoize=function(a,b){var c={};b||(b=h.identity);return function(){var f=b.apply(this,arguments);return h.has(c,f)?c[f]:c[f]=a.apply(this,arguments)}};h.delay=function(a,b){var c=g.call(arguments,2);return setTimeout(function(){return a.apply(null,c)},b)};h.defer=function(a){return h.delay.apply(h,[a,1].concat(g.call(arguments,1)))};h.throttle=function(a,b){var c,f,g,d,i,e,k=h.debounce(function(){i=d=!1},b);return function(){c=this;f=arguments;g||(g=setTimeout(function(){g=
null;i&&a.apply(c,f);k()},b));d?i=!0:e=a.apply(c,f);k();d=!0;return e}};h.debounce=function(a,b,c){var f;return function(){var g=this,d=arguments;c&&!f&&a.apply(g,d);clearTimeout(f);f=setTimeout(function(){f=null;c||a.apply(g,d)},b)}};h.once=function(a){var b=!1,c;return function(){if(b)return c;b=!0;return c=a.apply(this,arguments)}};h.wrap=function(a,b){return function(){var c=[a].concat(g.call(arguments,0));return b.apply(this,c)}};h.compose=function(){var a=arguments;return function(){for(var b=
arguments,c=a.length-1;0<=c;c--)b=[a[c].apply(this,b)];return b[0]}};h.after=function(a,b){return 0>=a?b():function(){if(1>--a)return b.apply(this,arguments)}};h.keys=H||function(a){if(a!==Object(a))throw new TypeError("Invalid object");var b=[],c;for(c in a)h.has(a,c)&&(b[b.length]=c);return b};h.values=function(a){return h.map(a,h.identity)};h.functions=h.methods=function(a){var b=[],c;for(c in a)h.isFunction(a[c])&&b.push(c);return b.sort()};h.extend=function(a){n(g.call(arguments,1),function(b){for(var c in b)a[c]=
b[c]});return a};h.pick=function(a){var b={};n(h.flatten(g.call(arguments,1)),function(c){c in a&&(b[c]=a[c])});return b};h.defaults=function(a){n(g.call(arguments,1),function(b){for(var c in b)null==a[c]&&(a[c]=b[c])});return a};h.clone=function(a){return!h.isObject(a)?a:h.isArray(a)?a.slice():h.extend({},a)};h.tap=function(a,b){b(a);return a};h.isEqual=function(a,b){return d(a,b,[])};h.isEmpty=function(a){if(null==a)return!0;if(h.isArray(a)||h.isString(a))return 0===a.length;for(var b in a)if(h.has(a,
b))return!1;return!0};h.isElement=function(a){return!!(a&&1==a.nodeType)};h.isArray=f||function(a){return"[object Array]"==k.call(a)};h.isObject=function(a){return a===Object(a)};h.isArguments=function(a){return"[object Arguments]"==k.call(a)};h.isArguments(arguments)||(h.isArguments=function(a){return!(!a||!h.has(a,"callee"))});h.isFunction=function(a){return"[object Function]"==k.call(a)};h.isString=function(a){return"[object String]"==k.call(a)};h.isNumber=function(a){return"[object Number]"==
k.call(a)};h.isFinite=function(a){return h.isNumber(a)&&isFinite(a)};h.isNaN=function(a){return a!==a};h.isBoolean=function(a){return!0===a||!1===a||"[object Boolean]"==k.call(a)};h.isDate=function(a){return"[object Date]"==k.call(a)};h.isRegExp=function(a){return"[object RegExp]"==k.call(a)};h.isNull=function(a){return null===a};h.isUndefined=function(a){return void 0===a};h.has=function(a,b){return j.call(a,b)};h.noConflict=function(){c._=e;return this};h.identity=function(a){return a};h.times=
function(a,b,c){for(var f=0;f<a;f++)b.call(c,f)};h.escape=function(a){return(""+a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#x27;").replace(/\//g,"&#x2F;")};h.result=function(a,b){if(null==a)return null;var c=a[b];return h.isFunction(c)?c.call(a):c};h.mixin=function(a){n(h.functions(a),function(b){var c=h[b]=a[b];t.prototype[b]=function(){var a=g.call(arguments);i.call(a,this._wrapped);return x(c.apply(h,a),this._chain)}})};var I=0;h.uniqueId=
function(a){var b=I++;return a?a+b:b};h.templateSettings={evaluate:/<%([\s\S]+?)%>/g,interpolate:/<%=([\s\S]+?)%>/g,escape:/<%-([\s\S]+?)%>/g};var y=/.^/,u={"\\":"\\","'":"'",r:"\r",n:"\n",t:"\t",u2028:"\u2028",u2029:"\u2029"},z;for(z in u)u[u[z]]=z;var J=/\\|'|\r|\n|\t|\u2028|\u2029/g,K=/\\(\\|'|r|n|t|u2028|u2029)/g,A=function(a){return a.replace(K,function(a,b){return u[b]})};h.template=function(a,b,c){c=h.defaults(c||{},h.templateSettings);a="__p+='"+a.replace(J,function(a){return"\\"+u[a]}).replace(c.escape||
y,function(a,b){return"'+\n_.escape("+A(b)+")+\n'"}).replace(c.interpolate||y,function(a,b){return"'+\n("+A(b)+")+\n'"}).replace(c.evaluate||y,function(a,b){return"';\n"+A(b)+"\n;__p+='"})+"';\n";c.variable||(a="with(obj||{}){\n"+a+"}\n");var a="var __p='';var print=function(){__p+=Array.prototype.join.call(arguments, '')};\n"+a+"return __p;\n",f=new Function(c.variable||"obj","_",a);if(b)return f(b,h);b=function(a){return f.call(this,a,h)};b.source="function("+(c.variable||"obj")+"){\n"+a+"}";return b};
h.chain=function(a){return h(a).chain()};var t=function(a){this._wrapped=a};h.prototype=t.prototype;var x=function(a,b){return b?h(a).chain():a};h.mixin(h);n("pop push reverse shift sort splice unshift".split(" "),function(b){var c=a[b];t.prototype[b]=function(){var a=this._wrapped;c.apply(a,arguments);var f=a.length;("shift"==b||"splice"==b)&&0===f&&delete a[0];return x(a,this._chain)}});n(["concat","join","slice"],function(b){var c=a[b];t.prototype[b]=function(){return x(c.apply(this._wrapped,arguments),
...
(function(){function d(a,b,c){if(a===b)return 0!==a||1/a==1/b;if(null==a||null==b)return a===b;a._chain&&(a=a._wrapped);b._chain&&(b=b._wrapped);if(a.isEqual&&h.isFunction(a.isEqual))return a.isEqual(b);if(b.isEqual&&h.isFunction(b.isEqual))return b.isEqual(a);var f=k.call(a);if(f!=k.call(b))return!1;switch(f){case "[object String]":return a==String(b);case "[object Number]":return a!=+a?b!=+b:0==a?1/a==1/b:a==+b;case "[object Date]":case "[object Boolean]":return+a==+b;case "[object RegExp]":return a.source==
b.source&&a.global==b.global&&a.multiline==b.multiline&&a.ignoreCase==b.ignoreCase}if("object"!=typeof a||"object"!=typeof b)return!1;for(var g=c.length;g--;)if(c[g]==a)return!0;c.push(a);var g=0,i=!0;if("[object Array]"==f){if(g=a.length,i=g==b.length)for(;g--&&(i=g in a==g in b&&d(a[g],b[g],c)););}else{if("constructor"in a!="constructor"in b||a.constructor!=b.constructor)return!1;for(var e in a)if(h.has(a,e)&&(g++,!(i=h.has(b,e)&&d(a[e],b[e],c))))break;if(i){for(e in b)if(h.has(b,e)&&!g--)break;
i=!g}}return c.pop(),i}var c=this,e=c._,b={},a=Array.prototype,f=Object.prototype,g=a.slice,i=a.unshift,k=f.toString,j=f.hasOwnProperty,l=a.forEach,r=a.map,m=a.reduce,s=a.reduceRight,B=a.filter,C=a.every,D=a.some,v=a.indexOf,E=a.lastIndexOf,f=Array.isArray,H=Object.keys,w=Function.prototype.bind,h=function(a){return new t(a)};"undefined"!=typeof exports?("undefined"!=typeof module&&module.exports&&(exports=module.exports=h),exports._=h):c._=h;h.VERSION="1.3.3";var n=h.each=h.forEach=function(a,c,
f){if(null!=a)if(l&&a.forEach===l)a.forEach(c,f);else if(a.length===+a.length)for(var g=0,d=a.length;g<d&&!(g in a&&c.call(f,a[g],g,a)===b);g++);else for(g in a)if(h.has(a,g)&&c.call(f,a[g],g,a)===b)break};h.map=h.collect=function(a,b,c){var f=[];return null==a?f:r&&a.map===r?a.map(b,c):(n(a,function(a,g,d){f[f.length]=b.call(c,a,g,d)}),a.length===+a.length&&(f.length=a.length),f)};h.reduce=h.foldl=h.inject=function(a,b,c,f){var g=2<arguments.length;null==a&&(a=[]);if(m&&a.reduce===m)return f&&(b=
h.bind(b,f)),g?a.reduce(b,c):a.reduce(b);n(a,function(a,d,i){g?c=b.call(f,c,a,d,i):(c=a,g=!0)});if(!g)throw new TypeError("Reduce of empty array with no initial value");return c};h.reduceRight=h.foldr=function(a,b,c,f){var g=2<arguments.length;null==a&&(a=[]);if(s&&a.reduceRight===s)return f&&(b=h.bind(b,f)),g?a.reduceRight(b,c):a.reduceRight(b);var d=h.toArray(a).reverse();return f&&!g&&(b=h.bind(b,f)),g?h.reduce(d,b,c,f):h.reduce(d,b)};h.find=h.detect=function(a,b,c){var f;return F(a,function(a,
g,d){if(b.call(c,a,g,d))return f=a,!0}),f};h.filter=h.select=function(a,b,c){var f=[];return null==a?f:B&&a.filter===B?a.filter(b,c):(n(a,function(a,g,d){b.call(c,a,g,d)&&(f[f.length]=a)}),f)};h.reject=function(a,b,c){var f=[];return null==a?f:(n(a,function(a,g,d){b.call(c,a,g,d)||(f[f.length]=a)}),f)};h.every=h.all=function(a,c,f){var g=!0;return null==a?g:C&&a.every===C?a.every(c,f):(n(a,function(a,d,i){if(!(g=g&&c.call(f,a,d,i)))return b}),!!g)};var F=h.some=h.any=function(a,c,f){c||(c=h.identity);
var g=!1;return null==a?g:D&&a.some===D?a.some(c,f):(n(a,function(a,d,i){if(g||(g=c.call(f,a,d,i)))return b}),!!g)};h.include=h.contains=function(a,b){var c=!1;return null==a?c:v&&a.indexOf===v?-1!=a.indexOf(b):(c=F(a,function(a){return a===b}),c)};h.invoke=function(a,b){var c=g.call(arguments,2);return h.map(a,function(a){return(h.isFunction(b)?b||a:a[b]).apply(a,c)})};h.pluck=function(a,b){return h.map(a,function(a){return a[b]})};h.max=function(a,b,c){if(!b&&h.isArray(a)&&a[0]===+a[0])return Math.max.apply(Math,
a);if(!b&&h.isEmpty(a))return-Infinity;var f={computed:-Infinity};return n(a,function(a,g,d){g=b?b.call(c,a,g,d):a;g>=f.computed&&(f={value:a,computed:g})}),f.value};h.min=function(a,b,c){if(!b&&h.isArray(a)&&a[0]===+a[0])return Math.min.apply(Math,a);if(!b&&h.isEmpty(a))return Infinity;var f={computed:Infinity};return n(a,function(a,g,d){g=b?b.call(c,a,g,d):a;g<f.computed&&(f={value:a,computed:g})}),f.value};h.shuffle=function(a){var b=[],c;return n(a,function(a,f){c=Math.floor(Math.random()*(f+
1));b[f]=b[c];b[c]=a}),b};h.sortBy=function(a,b,c){var f=h.isFunction(b)?b:function(a){return a[b]};return h.pluck(h.map(a,function(a,b,g){return{value:a,criteria:f.call(c,a,b,g)}}).sort(function(a,b){var c=a.criteria,f=b.criteria;return void 0===c?1:void 0===f?-1:c<f?-1:c>f?1:0}),"value")};h.groupBy=function(a,b){var c={},f=h.isFunction(b)?b:function(a){return a[b]};return n(a,function(a,b){var g=f(a,b);(c[g]||(c[g]=[])).push(a)}),c};h.sortedIndex=function(a,b,c){c||(c=h.identity);for(var f=0,g=
a.length;f<g;){var d=f+g>>1;c(a[d])<c(b)?f=d+1:g=d}return f};h.toArray=function(a){return a?h.isArray(a)?g.call(a):h.isArguments(a)?g.call(a):a.toArray&&h.isFunction(a.toArray)?a.toArray():h.values(a):[]};h.size=function(a){return h.isArray(a)?a.length:h.keys(a).length};h.first=h.head=h.take=function(a,b,c){return null!=b&&!c?g.call(a,0,b):a[0]};h.initial=function(a,b,c){return g.call(a,0,a.length-(null==b||c?1:b))};h.last=function(a,b,c){return null!=b&&!c?g.call(a,Math.max(a.length-b,0)):a[a.length-
1]};h.rest=h.tail=function(a,b,c){return g.call(a,null==b||c?1:b)};h.compact=function(a){return h.filter(a,function(a){return!!a})};h.flatten=function(a,b){return h.reduce(a,function(a,c){return h.isArray(c)?a.concat(b?c:h.flatten(c)):(a[a.length]=c,a)},[])};h.without=function(a){return h.difference(a,g.call(arguments,1))};h.uniq=h.unique=function(a,b,c){var c=c?h.map(a,c):a,f=[];return 3>a.length&&(b=!0),h.reduce(c,function(c,g,d){if(b?h.last(c)!==g||!c.length:!h.include(c,g))c.push(g),f.push(a[d]);
return c},[]),f};h.union=function(){return h.uniq(h.flatten(arguments,!0))};h.intersection=h.intersect=function(a){var b=g.call(arguments,1);return h.filter(h.uniq(a),function(a){return h.every(b,function(b){return 0<=h.indexOf(b,a)})})};h.difference=function(a){var b=h.flatten(g.call(arguments,1),!0);return h.filter(a,function(a){return!h.include(b,a)})};h.zip=function(){for(var a=g.call(arguments),b=h.max(h.pluck(a,"length")),c=Array(b),f=0;f<b;f++)c[f]=h.pluck(a,""+f);return c};h.indexOf=function(a,
b,c){if(null==a)return-1;var f;if(c)return f=h.sortedIndex(a,b),a[f]===b?f:-1;if(v&&a.indexOf===v)return a.indexOf(b);f=0;for(c=a.length;f<c;f++)if(f in a&&a[f]===b)return f;return-1};h.lastIndexOf=function(a,b){if(null==a)return-1;if(E&&a.lastIndexOf===E)return a.lastIndexOf(b);for(var c=a.length;c--;)if(c in a&&a[c]===b)return c;return-1};h.range=function(a,b,c){1>=arguments.length&&(b=a||0,a=0);for(var c=arguments[2]||1,f=Math.max(Math.ceil((b-a)/c),0),g=0,d=Array(f);g<f;)d[g++]=a,a+=c;return d};
var G=function(){};h.bind=function(a,b){var c,f;if(a.bind===w&&w)return w.apply(a,g.call(arguments,1));if(!h.isFunction(a))throw new TypeError;return f=g.call(arguments,2),c=function(){if(this instanceof c){G.prototype=a.prototype;var d=new G,i=a.apply(d,f.concat(g.call(arguments)));return Object(i)===i?i:d}return a.apply(b,f.concat(g.call(arguments)))}};h.bindAll=function(a){var b=g.call(arguments,1);return 0==b.length&&(b=h.functions(a)),n(b,function(b){a[b]=h.bind(a[b],a)}),a};h.memoize=function(a,
b){var c={};return b||(b=h.identity),function(){var f=b.apply(this,arguments);return h.has(c,f)?c[f]:c[f]=a.apply(this,arguments)}};h.delay=function(a,b){var c=g.call(arguments,2);return setTimeout(function(){return a.apply(null,c)},b)};h.defer=function(a){return h.delay.apply(h,[a,1].concat(g.call(arguments,1)))};h.throttle=function(a,b){var c,f,g,d,i,e,k=h.debounce(function(){i=d=!1},b);return function(){c=this;f=arguments;return g||(g=setTimeout(function(){g=null;i&&a.apply(c,f);k()},b)),d?i=!0:
e=a.apply(c,f),k(),d=!0,e}};h.debounce=function(a,b,c){var f;return function(){var g=this,d=arguments;c&&!f&&a.apply(g,d);clearTimeout(f);f=setTimeout(function(){f=null;c||a.apply(g,d)},b)}};h.once=function(a){var b=!1,c;return function(){return b?c:(b=!0,c=a.apply(this,arguments))}};h.wrap=function(a,b){return function(){var c=[a].concat(g.call(arguments,0));return b.apply(this,c)}};h.compose=function(){var a=arguments;return function(){for(var b=arguments,c=a.length-1;0<=c;c--)b=[a[c].apply(this,
b)];return b[0]}};h.after=function(a,b){return 0>=a?b():function(){if(1>--a)return b.apply(this,arguments)}};h.keys=H||function(a){if(a!==Object(a))throw new TypeError("Invalid object");var b=[],c;for(c in a)h.has(a,c)&&(b[b.length]=c);return b};h.values=function(a){return h.map(a,h.identity)};h.functions=h.methods=function(a){var b=[],c;for(c in a)h.isFunction(a[c])&&b.push(c);return b.sort()};h.extend=function(a){return n(g.call(arguments,1),function(b){for(var c in b)a[c]=b[c]}),a};h.pick=function(a){var b=
{};return n(h.flatten(g.call(arguments,1)),function(c){c in a&&(b[c]=a[c])}),b};h.defaults=function(a){return n(g.call(arguments,1),function(b){for(var c in b)null==a[c]&&(a[c]=b[c])}),a};h.clone=function(a){return h.isObject(a)?h.isArray(a)?a.slice():h.extend({},a):a};h.tap=function(a,b){return b(a),a};h.isEqual=function(a,b){return d(a,b,[])};h.isEmpty=function(a){if(null==a)return!0;if(h.isArray(a)||h.isString(a))return 0===a.length;for(var b in a)if(h.has(a,b))return!1;return!0};h.isElement=function(a){return!!a&&
1==a.nodeType};h.isArray=f||function(a){return"[object Array]"==k.call(a)};h.isObject=function(a){return a===Object(a)};h.isArguments=function(a){return"[object Arguments]"==k.call(a)};h.isArguments(arguments)||(h.isArguments=function(a){return!!a&&!!h.has(a,"callee")});h.isFunction=function(a){return"[object Function]"==k.call(a)};h.isString=function(a){return"[object String]"==k.call(a)};h.isNumber=function(a){return"[object Number]"==k.call(a)};h.isFinite=function(a){return h.isNumber(a)&&isFinite(a)};
h.isNaN=function(a){return a!==a};h.isBoolean=function(a){return!0===a||!1===a||"[object Boolean]"==k.call(a)};h.isDate=function(a){return"[object Date]"==k.call(a)};h.isRegExp=function(a){return"[object RegExp]"==k.call(a)};h.isNull=function(a){return null===a};h.isUndefined=function(a){return void 0===a};h.has=function(a,b){return j.call(a,b)};h.noConflict=function(){return c._=e,this};h.identity=function(a){return a};h.times=function(a,b,c){for(var f=0;f<a;f++)b.call(c,f)};h.escape=function(a){return(""+
a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#x27;").replace(/\//g,"&#x2F;")};h.result=function(a,b){if(null==a)return null;var c=a[b];return h.isFunction(c)?c.call(a):c};h.mixin=function(a){n(h.functions(a),function(b){var c=h[b]=a[b];t.prototype[b]=function(){var a=g.call(arguments);return i.call(a,this._wrapped),x(c.apply(h,a),this._chain)}})};var I=0;h.uniqueId=function(a){var b=I++;return a?a+b:b};h.templateSettings={evaluate:/<%([\s\S]+?)%>/g,
interpolate:/<%=([\s\S]+?)%>/g,escape:/<%-([\s\S]+?)%>/g};var y=/.^/,u={"\\":"\\","'":"'",r:"\r",n:"\n",t:"\t",u2028:"\u2028",u2029:"\u2029"},z;for(z in u)u[u[z]]=z;var J=/\\|'|\r|\n|\t|\u2028|\u2029/g,K=/\\(\\|'|r|n|t|u2028|u2029)/g,A=function(a){return a.replace(K,function(a,b){return u[b]})};h.template=function(a,b,c){c=h.defaults(c||{},h.templateSettings);a="__p+='"+a.replace(J,function(a){return"\\"+u[a]}).replace(c.escape||y,function(a,b){return"'+\n_.escape("+A(b)+")+\n'"}).replace(c.interpolate||
y,function(a,b){return"'+\n("+A(b)+")+\n'"}).replace(c.evaluate||y,function(a,b){return"';\n"+A(b)+"\n;__p+='"})+"';\n";c.variable||(a="with(obj||{}){\n"+a+"}\n");var a="var __p='';var print=function(){__p+=Array.prototype.join.call(arguments, '')};\n"+a+"return __p;\n",f=new Function(c.variable||"obj","_",a);if(b)return f(b,h);b=function(a){return f.call(this,a,h)};return b.source="function("+(c.variable||"obj")+"){\n"+a+"}",b};h.chain=function(a){return h(a).chain()};var t=function(a){this._wrapped=
a};h.prototype=t.prototype;var x=function(a,b){return b?h(a).chain():a};h.mixin(h);n("pop push reverse shift sort splice unshift".split(" "),function(b){var c=a[b];t.prototype[b]=function(){var a=this._wrapped;c.apply(a,arguments);var f=a.length;return("shift"==b||"splice"==b)&&0===f&&delete a[0],x(a,this._chain)}});n(["concat","join","slice"],function(b){var c=a[b];t.prototype[b]=function(){return x(c.apply(this._wrapped,arguments),this._chain)}});t.prototype.chain=function(){return this._chain=
...
"-"+a()+"-"+a()+"-"+a()+a()+a();localStorage.setItem(b,c._installationId)}return c._installationId};c._parseDate=function(b){b=/^([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2})T([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})(.([0-9]+))?Z$/.exec(b);return!b?null:new Date(Date.UTC(b[1]||0,(b[2]||1)-1,b[3]||0,b[4]||0,b[5]||0,b[6]||0,b[8]||0))};c._ajaxIE8=function(b,a,c,g,d){var e=new XDomainRequest;e.onload=function(){var a;try{a=JSON.parse(e.responseText)}catch(b){d&&d(e)}a&&g&&g(a,e)};e.onerror=e.ontimeout=function(){d(e)};
e.onprogress=function(){};e.open(b,a);e.send(c)};c._ajax=function(b,a,f,g,d){if("undefined"!=typeof XDomainRequest)return c._ajaxIE8(b,a,f,g,d);var e=!1,j=new XMLHttpRequest;j.onreadystatechange=function(){if(4===j.readyState&&!e)if(e=!0,200<=j.status&&300>j.status){var a;try{a=JSON.parse(j.responseText)}catch(b){d&&d(j)}a&&g&&g(a,j)}else d&&d(j)};j.open(b,a,!0);j.setRequestHeader("Content-Type","text/plain");j.send(f)};c._extend=function(b,a){var f,g=this,d;f=(b&&b.hasOwnProperty("constructor")?
d=b.constructor:d=function(){g.apply(this,arguments)},c._.extend(d,g),e.prototype=g.prototype,d.prototype=new e,b&&c._.extend(d.prototype,b),a&&c._.extend(d,a),d.prototype.constructor=d,d.__super__=g.prototype,d);return f.extend=this.extend,f};c._request=function(b,a,f,g,d,e){if(!c.applicationId)throw"You must specify your applicationId using Parse.initialize";if(!c.javaScriptKey&&!c.masterKey)throw"You must specify a key using Parse.initialize";if("classes"!==b&&"users"!==b&&"login"!==b&&"requestPasswordReset"!==
b)throw"First argument must be one of classes, users, or login, not '"+b+"'.";b=c.serverURL+"/1/"+b;a&&(b+="/"+a);f&&(b+="/"+f);d=c._.clone(d||{});"POST"!==g&&(d._method=g,g="POST");d._ApplicationId=c.applicationId;c._useMasterKey?d._MasterKey=c.masterKey:d._JavaScriptKey=c.javaScriptKey;d._ClientVersion="js"+c.VERSION;d._InstallationId=c._getInstallationId();(a=c.User.current())&&a._sessionToken&&(d._SessionToken=a._sessionToken);d=JSON.stringify(d);c._ajax(g,b,d,e.success,e.error)};c._getValue=
...
(function(){var d=this.Parse||(this.Parse={}),c=/\s+/,e=Array.prototype.slice;d.Events={on:function(b,a,f){var g,d,e,j,l;if(!a)return this;b=b.split(c);g=this._callbacks||(this._callbacks={});for(d=b.shift();d;)e=(l=g[d])?l.tail:{},e.next=j={},e.context=f,e.callback=a,g[d]={tail:j,next:l?l.next:e},d=b.shift();return this},off:function(b,a,f){var g,d,e,j,l,r;if(d=this._callbacks){if(!b&&!a&&!f)return delete this._callbacks,this;b=b?b.split(c):_.keys(d);for(g=b.shift();g;)if(e=d[g],delete d[g],e&&(a||
f)){j=e.tail;for(e=e.next;e!==j;)l=e.callback,r=e.context,(a&&l!==a||f&&r!==f)&&this.on(g,l,r),e=e.next;g=b.shift()}return this}},trigger:function(b){var a,f,g,d,k,j;if(!(g=this._callbacks))return this;k=g.all;b=b.split(c);j=e.call(arguments,1);for(a=b.shift();a;){if(f=g[a])for(d=f.tail;(f=f.next)!==d;)f.callback.apply(f.context||this,j);if(f=k){d=f.tail;for(a=[a].concat(j);(f=f.next)!==d;)f.callback.apply(f.context||this,a)}a=b.shift()}return this}};d.Events.bind=d.Events.on;d.Events.unbind=d.Events.off}).call(this);
...
a,g=b.latitude*a,a=this.longitude*a-b.longitude*a,b=Math.sin((c-g)/2),a=Math.sin(a/2),c=b*b+Math.cos(c)*Math.cos(g)*a*a;return c=Math.min(1,c),2*Math.asin(Math.sqrt(c))},kilometersTo:function(b){return 6371*this.radiansTo(b)},milesTo:function(b){return 3958.8*this.radiansTo(b)}}})(this);
(function(d){d.Parse=d.Parse||{};var c=d.Parse,e=c._;c.ACL=function(b){var a=this;a.permissionsById={};if(e.isObject(b))if(b instanceof c.User)a.setReadAccess(b,!0),a.setWriteAccess(b,!0);else{if(e.isFunction(b))throw"Parse.ACL() called with a function.  Did you forget ()?";c._each(b,function(b,g){if(!e.isString(g))throw"Tried to create an ACL with an invalid userId.";a.permissionsById[g]={};c._each(b,function(b,c){if("read"!==c&&"write"!==c)throw"Tried to create an ACL with an invalid permission type.";
if(!e.isBoolean(b))throw"Tried to create an ACL with an invalid permission value.";a.permissionsById[g][c]=b})})}};c.ACL.prototype.toJSON=function(){return e.clone(this.permissionsById)};c.ACL.prototype._setAccess=function(b,a,f){a instanceof c.User?a=a.id:a instanceof c.Role&&(a="role:"+a.getName());if(!e.isString(a))throw"userId must be a string.";if(!e.isBoolean(f))throw"allowed must be either true or false.";var g=this.permissionsById[a];if(!g){if(!f)return;g={};this.permissionsById[a]=g}f?this.permissionsById[a][b]=
!0:(delete g[b],e.isEmpty(g)&&delete g[a])};c.ACL.prototype._getAccess=function(b,a){a instanceof c.User?a=a.id:a instanceof c.Role&&(a="role:"+a.getName());var f=this.permissionsById[a];return f?f[b]?!0:!1:!1};c.ACL.prototype.setReadAccess=function(b,a){this._setAccess("read",b,a)};c.ACL.prototype.getReadAccess=function(b){return this._getAccess("read",b)};c.ACL.prototype.setWriteAccess=function(b,a){this._setAccess("write",b,a)};c.ACL.prototype.getWriteAccess=function(b){return this._getAccess("write",
...
null;var f=this,g=function(a){if(a instanceof c.Object){if(!a.id)throw"You can't add an unsaved Parse.Object to a relation.";f._targetClassName||(f._targetClassName=a.className);if(f._targetClassName!==a.className)throw"Tried to create a Parse.Relation with 2 different types: "+f._targetClassName+" and "+a.className+".";return a.id}return a};this.relationsToAdd=e.uniq(e.map(b,g));this.relationsToRemove=e.uniq(e.map(a,g))},added:function(){var b=this;return e.map(this.relationsToAdd,function(a){var f=
c.Object._create(b._targetClassName);return f.id=a,f})},removed:function(){var b=this;return e.map(this.relationsToRemove,function(a){var f=c.Object._create(b._targetClassName);return f.id=a,f})},toJSON:function(){var b=null,a=null,c=this,g=function(a){return{__type:"Pointer",className:c._targetClassName,objectId:a}},d=null;return 0<this.relationsToAdd.length&&(d=e.map(this.relationsToAdd,g),b={__op:"AddRelation",objects:d}),0<this.relationsToRemove.length&&(d=e.map(this.relationsToRemove,g),a={__op:"RemoveRelation",
...
(function(d){d.Parse=d.Parse||{};var c=d.Parse,e=c._;c.Object=function(a,b){if(e.isString(a))return c.Object._create.apply(this,arguments);a=a||{};b&&b.parse&&(a=this.parse(a));var g=c._getValue(this,"defaults");g&&(a=e.extend({},g,a));b&&b.collection&&(this.collection=b.collection);this._serverData={};this._opSetQueue=[{}];this.attributes={};this._hashedJSON={};this._escapedAttributes={};this.cid=e.uniqueId("c");this.changed={};this._silent={};this._pending={};if(!this.set(a,{silent:!0}))throw Error("Can't create an invalid Parse.Object");
this.changed={};this._silent={};this._pending={};this._hasData=!0;this._previousAttributes=e.clone(this.attributes);this.initialize.apply(this,arguments)};var b=function(a,c,g,d){var g=g||[],k;k=e.isFunction(d)?{success:function(a){d(a,null)},error:function(a){d(null,a)}}:d;if(a.length){var j=k?e.clone(k):{};j.success=function(d){g.push(d);b(e.rest(a),c,g,k)};c.call(this,e.first(a),j)}else k.success&&k.success(g)};c.Object.saveAll=function(a,c){b(a,function(a,b){a.save(null,b)},[],c)};c.Object._signUpAll=
function(a,c){b(a,function(a,b){a.signUp(null,b)},[],c)};e.extend(c.Object.prototype,c.Events,{_existed:!1,initialize:function(){},toJSON:function(){var a=this._toFullJSON();return e.each(["createdAt","objectId","updatedAt","__type","className"],function(b){delete a[b]}),a},_toFullJSON:function(a){var b=e.clone(this.attributes);return c._each(b,function(g,d){b[d]=c._encode(g,a)}),c._each(this._operations,function(a,c){b[c]=a}),b.objectId=this.id,b.createdAt=this.createdAt,b.updatedAt=this.updatedAt,
b.__type="Object",b.className=this.className,b},_refreshCache:function(){var a=this;c._each(this.attributes,function(b,g){b instanceof c.Object?b._refreshCache():e.isObject(b)&&a._resetCacheForKey(g)&&a.set(g,new c.Op.Set(b),{silent:!0})})},dirty:function(a){this._refreshCache();var b=e.last(this._opSetQueue);return a?b[a]?!0:!1:this.id?0<e.keys(b).length?!0:!1:!0},_toPointer:function(){if(!this.id)throw Error("Can't serialize an unsaved Parse.Object");return{__type:"Pointer",className:this.className,
objectId:this.id}},get:function(a){return this.attributes[a]},relation:function(a){var b=this.get(a);if(b){if(b instanceof c.Relation)return b._ensureParentAndKey(this,a),b;throw"Called relation() on non-relation field "+a;}return new c.Relation(this,a)},escape:function(a){var b=this._escapedAttributes[a];if(b)return b;var b=this.attributes[a],g;return c._isNullOrUndefined(b)?g="":g=e.escape(b.toString()),this._escapedAttributes[a]=g,g},has:function(a){return!c._isNullOrUndefined(this.attributes[a])},
...
c._each(a,function(c,d){var e=a[d],j=b[d];e&&j?b[d]=j._mergeWithPrevious(e):e&&(b[d]=e)});this._processSaveQueue()},_finishSave:function(a){var b=e.first(this._opSetQueue);this._opSetQueue=e.rest(this._opSetQueue);this._applyOpSet(b,this._serverData);this._mergeMagicFields(a);var g=this;c._each(a,function(a,b){g._serverData[b]=c._decode(b,a)});this._rebuildAllEstimatedData();this._processSaveQueue()},_finishFetch:function(a,b){this._mergeMagicFields(a);var g=this;c._each(a,function(a,b){g._serverData[b]=
c._decode(b,a)});this._refreshCache();this._hasData=b;this._opSetQueue=[{}];this._rebuildAllEstimatedData()},_applyOpSet:function(a,b){var g=this;c._.each(a,function(a,d){b[d]=a._estimate(b[d],g,d);b[d]===c.Op._UNSET&&delete b[d]})},_resetCacheForKey:function(a){var b=this.attributes[a];return e.isObject(b)&&!(b instanceof c.Object)&&(b=b.toJSON?b.toJSON():b,b=JSON.stringify(b),this._hashedJSON[a]!==b)?(this._hashedJSON[a]=b,!0):!1},_rebuildEstimatedDataForKey:function(a){var b=this;delete this.attributes[a];
this._serverData[a]&&(this.attributes[a]=this._serverData[a]);e.each(this._opSetQueue,function(g){(g=g[a])&&(b.attributes[a]=g._estimate(b.attributes[a],b,a),b.attributes[a]===c.Op._UNSET?delete b.attributes[a]:b._resetCacheForKey(a))})},_rebuildAllEstimatedData:function(){var a=this;this.attributes=e.clone(this._serverData);e.each(this._opSetQueue,function(b){a._applyOpSet(b,a.attributes);e.each(b,function(b,c){a._resetCacheForKey(c)})})},set:function(a,b,g){var d;e.isObject(a)||c._isNullOrUndefined(a)?
(d=a,c._each(d,function(a,b){d[b]=c._decode(b,a)}),g=b):(d={},d[a]=c._decode(a,b));g=g||{};if(!d)return this;d instanceof c.Object&&(d=d.attributes);g.unset&&c._each(d,function(a,b){d[b]=new c.Op.Unset});var k=e.clone(d),j=this;c._each(k,function(a,b){a instanceof c.Op&&(k[b]=a._estimate(j.attributes[b],j,b),k[b]===c.Op._UNSET&&delete k[b])});if(!this._validate(d,g))return!1;this._mergeMagicFields(d);g.changes={};var l=this._escapedAttributes;return c._each(e.keys(d),function(a){var b=d[a];b instanceof
c.Relation&&(b.parent=j);b instanceof c.Op||(b=new c.Op.Set(b));var f=!0;b instanceof c.Op.Set&&e.isEqual(j.attributes[a],b.value)&&(f=!1);f&&(delete l[a],g.silent?j._silent[a]=!0:g.changes[a]=!0);var k=e.last(j._opSetQueue);k[a]=b._mergeWithPrevious(k[a]);j._rebuildEstimatedDataForKey(a);f?(j.changed[a]=j.attributes[a],g.silent||(j._pending[a]=!0)):(delete j.changed[a],delete j._pending[a])}),g.silent||this.change(g),this},unset:function(a,b){return b=b||{},b.unset=!0,this.set(a,null,b)},increment:function(a,
...
(function(d){d.Parse=d.Parse||{};var c=d.Parse,e=c._;c.Query=function(b){e.isString(b)&&(b=c.Object._getSubclass(b));this.objectClass=b;this.className=b.prototype.className;this._where={};this._include=[];this._limit=-1;this._skip=0};c.Query.prototype={get:function(b,a){var d=a.success||function(){},g=a.error||function(){};this.equalTo("objectId",b);this.first({error:function(a){g(null,a)},success:function(a){a?d(a):g(null,new c.Error(c.Error.OBJECT_NOT_FOUND,"Object not found."))}})},toJSON:function(){var b=
{where:this._where};return 0<this._include.length&&(b.include=this._include.join(",")),0<=this._limit&&(b.limit=this._limit),0<this._skip&&(b.skip=this._skip),void 0!==this._order&&(b.order=this._order),b},find:function(b){var a=this,d=b.success||function(){},g={error:b.error,success:function(b){d(e.map(b.results,function(b){var c=new a.objectClass;return c._finishFetch(b,!0),c}))}},i=this.toJSON();g.error=c.Query._wrapError(b.error,g);c._request("classes",this.className,null,"GET",i,g)},count:function(b){var a=
b.success||function(){},d={error:b.error,success:function(b){a(b.count)}},g=this.toJSON();g.limit=0;g.count=1;d.error=c.Query._wrapError(b.error,d);c._request("classes",this.className,null,"GET",g,d)},first:function(b){var a=this,d=b.success||function(){},g={error:b.error,success:function(b){d(e.map(b.results,function(b){var c=new a.objectClass;return c._finishFetch(b,!0),c})[0])}},i=this.toJSON();i.limit=1;g.error=c.Query._wrapError(b.error,g);c._request("classes",this.className,null,"GET",i,g)},
...
this._include=this._include.concat(b):this._include.push(b),this}};c.Query._wrapError=function(b,a){return function(d){if(b){var g=new c.Error(-1,d.responseText);d.responseText&&(d=JSON.parse(d.responseText))&&(g=new c.Error(d.code,d.error));b(g,a)}}}})(this);
...
(a=a.substr(this.options.root.length)),a.replace(b,"")},start:function(d){if(c.History.started)throw Error("Parse.history has already been started");c.History.started=!0;this.options=e.extend({},{root:"/"},this.options,d);this._wantsHashChange=!1!==this.options.hashChange;this._wantsPushState=!!this.options.pushState;this._hasPushState=!(!this.options.pushState||!window.history||!window.history.pushState);var d=this.getFragment(),g=document.documentMode;(g=a.exec(navigator.userAgent.toLowerCase())&&
(!g||7>=g))&&(this.iframe=c.$('<iframe src="javascript:0" tabindex="-1" />').hide().appendTo("body")[0].contentWindow,this.navigate(d));this._hasPushState?c.$(window).bind("popstate",this.checkUrl):this._wantsHashChange&&"onhashchange"in window&&!g?c.$(window).bind("hashchange",this.checkUrl):this._wantsHashChange&&(this._checkUrlInterval=window.setInterval(this.checkUrl,this.interval));this.fragment=d;d=window.location;g=d.pathname===this.options.root;if(this._wantsHashChange&&this._wantsPushState&&
!this._hasPushState&&!g)return this.fragment=this.getFragment(null,!0),window.location.replace(this.options.root+"#"+this.fragment),!0;this._wantsPushState&&this._hasPushState&&g&&d.hash&&(this.fragment=this.getHash().replace(b,""),window.history.replaceState({},document.title,d.protocol+"//"+d.host+this.options.root+this.fragment));if(!this.options.silent)return this.loadUrl()},stop:function(){c.$(window).unbind("popstate",this.checkUrl).unbind("hashchange",this.checkUrl);window.clearInterval(this._checkUrlInterval);
...
function(a,b){return 0==this.itemToString(a).toLowerCase().indexOf(b.toLowerCase())};p.stringToItem=function(a){return a};p.itemToString=function(a){return a};p.compareItems=function(a,b){return a==b};p=c.prototype;p.init=function(a,b){var c=this,f,e,g;c._defaults=d.extend({},l);c._opts=b||{};c._plugins={};c._itemManager=e=new (c.opts("item.manager"));a=d(a);g=d(c.opts("html.wrap"));f=d(c.opts("html.hidden"));a.wrap(g).keydown(function(a){return c.onKeyDown(a)}).keyup(function(a){return c.onKeyUp(a)}).data("textext",
...
c.trigger("ready");c.getFormData(0)};p.initPatches=function(){var a=[],b=d.fn.textext.patches,c;for(c in b)a.push(c);this.initPlugins(a,b)};p.initPlugins=function(a,b){var c,f,e=[],g;"string"==typeof a&&(a=a.split(/\s*,\s*|\s+/g));for(g=0;g<a.length;g++)c=a[g],(f=b[c])&&(this._plugins[c]=f=new f,this[c]=function(a){return function(){return a}}(f),e.push(f),d.extend(!0,f,this.opts("ext.*"),this.opts("ext."+c)));e.sort(function(a,b){return a=a.initPriority(),b=b.initPriority(),a===b?0:a<b?1:-1});for(g=
...
a.query))}})(jQuery);
(function(d){function c(){}d.fn.textext.TextExtTags=c;d.fn.textext.addPlugin("tags",c);var e=c.prototype,b="text-tag",a="."+b,f=".text-label",g={tags:{enabled:!0,items:null},html:{tags:'<div class="text-tags"/>',tag:'<div class="text-tag"><div class="text-button"><span class="text-label"/><a class="text-remove"/></div></div>'}};e.init=function(a){this.baseInit(a,g);var a=this.input(),b;this.opts("tags.enabled")&&(b=d(this.opts("html.tags")),a.after(b),d(this).data("container",b),this.on({enterKeyPress:this.onEnterKeyPress,
